...
// 流星
class ShootingStar {
    constructor(init = new Crood, final = new Crood, size = 3, speed = 200, length = 1110, onDistory = null) {
        this.init = init; // 初始位置
    ...
    draw(ctx, delta) {
        ...
        this.now.setCrood(
            this.init.x + (this.final.x - this.init.x) * percent * 5,
            this.init.y + (this.final.y - this.init.y) * percent * 5
        );
...
class MeteorShower {
    ...
    createStar() {
        ...
        let length = Math.random() * 2 + 100;
        let star = new ShootingStar(
            init, final, size, speed, length,
            () => {
                this.remove(star)
            }
        );
    ...
    update(delta) {
        if (!this.stop && this.stars.length < 10) {
    ...
    tick() {
        ...
        let _tick = () => {
            ...
            ctx.save();
            ctx.globalCompositeOperation = "destination-in"; // 在源图像中显示目标图像。只有源图像内的目标图像部分会被显示，源图像是透明的。
            ctx.fillStyle = 'rgba(0,0,0,0.85)'; // 每一帧用 “半透明” 的背景色清除画布
